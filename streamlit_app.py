...
# 
    ...
    @st.cache_data
    def load_dataset():
        return pd.read_csv("LLM_related_domainss.csv")
    ...
    if st.button("Confirm and Extract Topics"):
        if user_title and user_abstract:
            ...
            st.write("### Extracted Topics and Suggestions")
            st.text_area("Topics:", analysis, height=300)

            # Load dataset and match topics
            st.write("### Matched Topics from Dataset")
            dataset = load_dataset()
            matched_topics = dataset[dataset['Human_Readable_Topics'].str.contains("|".join(analysis.split('\n')), na=False, case=False)]
            if not matched_topics.empty:
                st.dataframe(matched_topics)
            else:
                st.write("No matching topics found in the dataset.")
        else:
